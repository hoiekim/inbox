...
Mail.searchMail = (value, field) => {
  value = value.replaceAll("<", "").replaceAll(">", "");

  const reservedCahracters = [
    "+",
    "-",
    "=",
    "&&",
  ...
  const heightlight = { fields: {} };
  const fields = field ? [field] : ["subject", "text"];
  fields.forEach((e, i) => {
    heightlight.fields[e] = {};
    fields[i] = e + "^" + fields.length - i;
  });

  return Mail.request("_search", "GET", {
