...
db.getAccounts = () => {
  const accounts = sendESRequest(
    `/${ELASTIC_INDEX}/_msearch`,
    "POST",
    [
      {},
      {
  ...
  return accounts.then((r) => {
    ...
    allAccounts.forEach((e) => {
      const unreadMailsCount = unreadAccounts.find((f) => e.key === f.key);
      e.unread_doc_count = unreadMailsCount?.doc_count || 0;
    });
    allAccounts.sort((a, b) => (a.key > b.key ? 1 : b.key > a.key ? -1 : 0));
    return { received: allAccounts, sent: sentAccounts };
  });
};
