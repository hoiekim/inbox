  ...
  try {
    ...
    switch (cmd) {
      case Command.CAPABILITY:
        return session.write(`* CAPABILITY IMAP4rev1 LITERAL+ SASL-IR LOGIN-REFERRALS ID ENABLE IDLE STARTTLS AUTH=PLAIN\r\n${tag} OK CAPABILITY completed\r\n`);
      ...
      case Command.STATUS:
        ...
        if (args.length < 2) {
          return session.write(`${tag} BAD STATUS requires mailbox name and status items\r\n`);
        }
      ...
      case Command.CLOSE:
        ...
        return session.write(`${tag} OK CLOSE completed\r\n`);
...
const parseCommand = (line: string): { tag: string; command: string; args: string[] } => {
  const parts: string[] = [];
  let current = "";
  let inQuotes = false;
  let inLiteral = false;
...
export const imapListener = (socket: Socket) => {
  ...
  socket.write("* OK [CAPABILITY IMAP4rev1 LITERAL+ SASL-IR LOGIN-REFERRALS ID ENABLE IDLE STARTTLS AUTH=PLAIN] IMAP4rev1 Service Ready\r\n");
