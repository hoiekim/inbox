 ...
 *     across TCP segmentation and both literal forms, for any payload that
 *     decodes as UTF-8. Octets that are not valid UTF-8 — legal in an APPEND
 *     body per RFC 3501 §4.3 — become U+FFFD at the framing boundary's
 *     decode, and nothing here covers them.
...
describe("IMAP literal continuation", () => {
  ...
  it("keeps a final APPEND payload dispatching when its trailing CRLF arrives separately", async () => {
    ...
    // A final literal whose payload lands alone: the tail CRLF completes the
    // command on the next segment.
    const message = "Subject: x\r\n\r\nbody";
    socket.emit("data", Buffer.from(`a1 APPEND INBOX {${Buffer.byteLength(message)}+}\r\n`));
    await tick();
  ...
  it("answers a literal-declaring command pipelined behind a self-terminated payload", async () => {
    const { socket, dispatched } = makeHarness();

    socket.emit(
      "data",
      Buffer.from("a1 APPEND INBOX {13+}\r\nHello World\r\na2 SELECT {5+}\r\nINBOX\r\n")
    );
    await settle();

    expect(dispatched.map((d) => d.tag)).toEqual(["a1", "a2"]);
    expect(dispatched[1].request).toEqual({
      type: "SELECT",
      data: { mailbox: "INBOX" }
    });
  });

  it("still chains a separated literal onto a command that already parses", async () => {
    const { socket, dispatched } = makeHarness();

    // `SEARCH SUBJECT {3+}` + ["foo"] is a complete command on its own, so
    // completeness alone cannot decide whether the tail continues it. The
    // separating SP does: an argument tail carries one, a pipelined command
    // sits flush against the buffer start.
    socket.emit(
      "data",
      Buffer.from("A1 SEARCH SUBJECT {3+}\r\nfoo FROM {3+}\r\nbar\r\n")
    );
    await settle();
